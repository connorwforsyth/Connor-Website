...
p2: The outcome identified several improvements that could be made to key customer tasks within the chatbot and IVR (interactive voice response) systems.
protected: true
---
...
- We conducted a heuristic evaluation of critical user tasks across three chatbots.
- We benchmarked the chatbot tasks against chosen metrics and heuristics.
...
To get our heads around how we would best conduct our analysis, we leveraged principles of conversation design[^1]. 
...
We approached the heuristic evaluation through a task-based lens. In conversation design, tasks are commonly referred to as intents[^2], not to be confused with utterances[^3].
<br/>
...
Three of us ran the evaluation to minimise bias and obtain a range of data for the test results.

Using templated databases within Notion enabled us to set up a consistent testing file that was linked back to master databases.

This also meant that we could roll up the results from each test to do some high-level summarisation of each of the intents, heuristics, and severity.
...
Additionally, we delivered the heuristic evaluation as raw CSV data, suitable for the client to upload into their backlog and start working with in their agile teams.
<br/>
...
- Different patterns in UX that influence e-commerce + organisations.
</div>
<div>
Loathed 😞
...
[^4]: They are recognised usability principles. Heuristics help us identify and categorise usability problems found within the design of a digital product. We examine the product and assess it against a chosen set of heuristics.